    ...
    <div class="container nuevas-recetas">
        ...
        <div class="owl-carousel owl-theme">
            @foreach ($nuevas as $nueva)
                <div class="card">
                    <img src="/storage/{{ $nueva->imagen }}" alt="imagen-card" class="card-img-top">
                    <div class="card-body">
                        <h3 class="card-title">{{ $nueva->titulo }}</h3>
                        <p>{{ Str::words(strip_tags($nueva->preparacion), 10) }}</p>
                        <a href="{{ route('recetas.show', ['receta' => $nueva->id]) }}"
                            class="btn btn-primary font-weight-bold d-block text-uppercase">Ver Receta</a>
                    </div>
                </div>
            @endforeach
        </div>
